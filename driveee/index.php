...
$yakutiaAddresses = [
    ...
    'Якутск, ул. Дзержинского, 12', 'Якутск, мкр. Марха, ул. Лесная, 3', 'Якутск, мкр. Сайсары, ул. Тимирязева, 7',
    'Якутск, ул. Чернышевского, 25', 'Мирный, ул. Советская, 10', 'Мирный, ул. Ленина, 5',
    'Нерюнгри, пр. Дружбы, 15', 'Нерюнгри, ул. Карла Маркса, 8', 'Алдан, ул. Космонавтов, 3', 'Алдан, ул. Ленина, 12'
];
?>
<!DOCTYPE html>
<html lang="ru">
<head>
    ...
    <title>DRIVEEE - Якутия</title>
    <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" />
    <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        body { background: #1a1a1a; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; }
        ...
        .app-header h1 { font-size: 22px; font-weight: 700; color: #00cc44; }
        ...
        .price-input { width: 100%; padding: 14px; border: 1px solid #e0e0e0; border-radius: 30px; font-size: 15px; margin: 16px 0; }
        .order-btn { width: 100%; padding: 16px; background: #00cc44; border: none; border-radius: 30px; font-size: 17px; font-weight: 700; color: #1a1a1a; cursor: pointer; }
        ...
        .exit-btn { width: 100%; padding: 14px; background: #ff6600; color: white; border: none; border-radius: 30px; font-weight: bold; margin-top: 16px; display: none; cursor: pointer; }
        ...
        .driver-stat { flex: 1; background: white; padding: 16px; border-radius: 20px; text-align: center; }
        ...
        .priority-tag { display: inline-block; background: #ff6600; color: white; padding: 2px 10px; border-radius: 20px; font-size: 11px; margin-left: 8px; }
        ...
        .passenger-onboard-btn { background: #00cc44; color: #1a1a1a; }
        .complete-btn { background: #1a1a1a; color: white; }
    ...
    <div class="app-content">
        <?php if (!$isLoggedIn): ?>
            <div style="padding: 40px 20px; text-align: center;">
                ...
                <p style="color: #666; margin-bottom: 30px;">Такси по Якутии</p>
        ...
        <?php elseif ($userData['user_type'] == 'passenger'): ?>
            ...
            <div class="order-panel">
                ...
                <input type="number" id="ridePrice" class="price-input" placeholder="Сумма поездки (₽)" value="300">
                <label style="display: flex; align-items: center; gap: 8px; margin-bottom: 16px;"><input type="checkbox" id="priorityCheckbox"> 🚀 Приоритетная поездка</label>
                ...
                <button id="exitBtn" class="exit-btn">🚪 Выхожу</button>
        ...
        <?php elseif ($userData['user_type'] == 'driver'): ?>
            ...
            <div class="order-panel">
                <div class="stats-row">
                    ...
                    <div class="streak-badge">🚗 <?php echo $driverData['total_rides'] ?? 0; ?> поездок</div>
                </div>
...
function findDriverLocation() {
    if (navigator.geolocation && driverMap) {
        navigator.geolocation.getCurrentPosition(function(p) {
            driverMap.setView([p.coords.latitude, p.coords.longitude], 14);
        });
    }
}
...
function showAddressSuggestions(inputId, query) {
    let div = document.getElementById(inputId + 'Suggestions');
    if(!query || query.length < 2) { div.style.display = 'none'; return; }
    
    let filtered = yakutiaAddresses.filter(function(addr) {
...
function orderRide() {
    ...
    if(!pickup || !dropoff) { alert('Укажите адреса'); return; }
    if(!price || price <= 0) { alert('Укажите сумму'); return; }
...
function startOrderCheck() { 
    ...
    checkInterval = setInterval(checkOrder, 2000); 
}
    ...
    fetch('/api/orders.php?action=check&order_id=' + currentOrderId)
        ...
        .then(function(data){
            ...
            if(data.status === 'accepted') {
                ...
                showNotification('✅ Поездка завершена! Огонек +1');
...
document.getElementById('exitBtn').onclick = function() {
    fetch('/api/orders.php', {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({action:'exit', order_id:currentOrderId})
    }).then(function(r) { return r.json(); }).then(function(data){
        if(data.success) {
            document.getElementById('exitBtn').style.display = 'none';
            startTimer();
    ...
    timerInterval = setInterval(function(){
        ...
        if(timeLeft <= 0){
            ...
            fetch('/api/orders.php',{
                method:'POST',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({action:'late', order_id:currentOrderId})
            });
        }
    }, 1000);
}
...
function updateTimerDisplay() { 
    ...
    document.getElementById('timerValue').textContent = m + ':' + (s<10?'0'+s:s); 
}
...
function loadAvailableOrders() {
    fetch('/api/orders.php?action=pending')
        .then(function(r) { return r.json(); })
        .then(function(data){
            var container = document.getElementById('availableOrders');
            if(container && data.orders){
                if(data.orders.length === 0) {
                    container.innerHTML = '<div style="text-align:center;padding:20px;color:#888;">📭 Нет заказов</div>';
                } else {
                    var html = '';
                    for(var i = 0; i < data.orders.length; i++) {
                        var o = data.orders[i];
                        html += '<div class="order-card ' + (o.is_priority ? 'priority' : '') + '">';
                        html += '<div><strong>📍</strong> ' + escapeHtml(o.pickup_address) + '</div>';
                        html += '<div><strong>🏁</strong> ' + escapeHtml(o.dropoff_address) + '</div>';
                        html += '<div><strong>💰</strong> ' + o.offered_price + ' ₽ ' + (o.is_priority ? '<span class="priority-tag">🚀 ПРИОРИТЕТ</span>' : '') + '</div>';
                        html += '<div><strong>👤</strong> ' + escapeHtml(o.passenger_name) + '</div>';
...
function acceptOrder(orderId, price){
    fetch('/api/orders.php',{
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({action:'accept', order_id:orderId, final_price:price})
    }).then(function(r) { return r.json(); }).then(function(data){ 
        alert(data.message); 
        if(data.success){ 
            loadAvailableOrders(); 
...
function loadMyOrders() {
    fetch('/api/orders.php?action=my')
        .then(function(r) { return r.json(); })
        .then(function(data){
            var container = document.getElementById('myOrders');
            if(container && data.orders){
                if(data.orders.length === 0) {
                    container.innerHTML = '<div style="text-align:center;padding:20px;color:#888;">📭 Нет активных заказов</div>';
                } else {
                    var html = '';
                    for(var i = 0; i < data.orders.length; i++) {
                        var o = data.orders[i];
                        ...
                        html += '<div><strong>📍</strong> ' + escapeHtml(o.pickup_address) + '</div>';
                        html += '<div><strong>🏁</strong> ' + escapeHtml(o.dropoff_address) + '</div>';
                        html += '<div><strong>💰</strong> ' + (o.final_price || o.offered_price) + ' ₽</div>';
                        html += '<div><strong>👤</strong> ' + escapeHtml(o.passenger_name) + '</div>';
                        html += '<div><strong>Статус:</strong> ' + o.status + '</div>';
                        html += '<div class="action-btns">';
                        
                        // Кнопка "Подъехал" - если статус accepted
                        if(o.status === 'accepted') {
                            html += '<button class="action-btn arrived-btn" onclick="updateOrder(' + o.id + ',\'arrived\')">🚗 Подъехал</button>';
                        }
                        
                        // Кнопка "Пассажир сел" - если статус waiting_exit
                        if(o.status === 'waiting_exit') {
                            html += '<button class="action-btn passenger-onboard-btn" onclick="updateOrder(' + o.id + ',\'passenger_onboard\')">👤 Пассажир сел</button>';
                        }
                        
                        // Кнопка "Завершить поездку" - если статус passenger_onboard
                        if(o.status === 'passenger_onboard') {
                            html += '<button class="action-btn complete-btn" onclick="updateOrder(' + o.id + ',\'complete\')">✅ Завершить поездку</button>';
                        }
                        
                        // Кнопка отмены для всех статусов, кроме completed
...
function updateOrder(orderId, action){
    console.log('updateOrder called:', orderId, action);
    ...
    fetch('/api/orders.php',{
        ...
        if(data.success){ 
            if(action === 'complete') {
                alert('🎉 Поездка завершена! +10 Драйвикоинов');
                location.reload();
            } else {
                loadMyOrders();
                loadAvailableOrders();
...
function showTab(tab){
    var avail = document.getElementById('availableOrders');
    var my = document.getElementById('myOrders');
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    var pu = document.getElementById('pickupAddress');
    var dof = document.getElementById('dropoffAddress');
    
    if(pu){ 
        pu.addEventListener('input', function(e) { showAddressSuggestions('pickup', e.target.value); });
        pu.addEventListener('blur', function() { setTimeout(function() { document.getElementById('pickupSuggestions').style.display = 'none'; }, 300); });
    }
    if(dof){ 
        dof.addEventListener('input', function(e) { showAddressSuggestions('dropoff', e.target.value); });
        dof.addEventListener('blur', function() { setTimeout(function() { document.getElementById('dropoffSuggestions').style.display = 'none'; }, 300); });
    }
